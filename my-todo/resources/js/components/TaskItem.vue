<template>
    <li class="flex items-center space-x-2 p-3 border-b w-11/12">
        <div class="flex items-center space-x-4 w-full">
            ...
            <span @click="handleToggleTaskStatus" class="cursor-pointer" :class="statusClass">{{ t(`status.${task.status}`) }}</span>
...
const statusClass = computed(() => {
    return {
        "bg-gray-300 px-2 py-1 rounded": props.task.status === "todo",
        "bg-yellow-300 px-2 py-1 rounded": props.task.status === "doing",
        "bg-green-500 px-2 py-1 rounded text-white": props.task.status === "done"
    };
});
